...
export default function MaintenanceGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/maintenance').then((res) => res.json()).catch(() => null),
      fetch('/api/auth/me').then((res) => res.json()).catch(() => null),
    ]).then(([maintData, authData]) => {
      if (!isCurrent) return;
  ...
  // If on super-admin or maintenance page, allow pass-through
  if (isBypassedRoute) {
    return <>{children}</>;
  }

  // If maintenance is enabled and user is not Super Admin, show maintenance screen
  if (checked && maintenance?.enabled && !isSuperAdmin) {
    const formattedRestorationTime = maintenance.estimatedRestorationTime
      ? new Date(maintenance.estimatedRestorationTime).toLocaleString('en-US', {
          dateStyle: 'full',
          timeStyle: 'short',
        })
      : null;
    ...
    return (
      <div className="min-h-screen bg-radial from-slate-900 via-[#0a1910] to-[#04120a] text-slate-100 flex flex-col justify-between p-4 sm:p-6 font-sans relative overflow-hidden">
          ...
          <div className="bg-slate-900/90 backdrop-blur-md border border-emerald-900/40 rounded-3xl p-6 sm:p-10 shadow-2xl space-y-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <h1 className="text-2xl sm:text-4xl font-serif font-black text-white tracking-tight">
                {maintenance.heading || 'We’ll Be Back Soon!'}
              </h1>
            </div>

            {/* MESSAGE CONTENT */}
            <div className="text-slate-300 text-sm sm:text-base leading-relaxed whitespace-pre-line bg-slate-950/50 p-5 sm:p-6 rounded-2xl border border-slate-800 text-left font-medium">
              {maintenance.message}
            </div>
