...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      prisma.donation.aggregate({ where: { masjidId, isVoided: false }, _sum: { amount: true }, _count: true }).catch(() => ({ _sum: { amount: 0 }, _count: 0 })),
      prisma.donation.aggregate({ where: { masjidId, isVoided: false, date: { gte: currentMonthStart } }, _sum: { amount: true }, _count: true }).catch(() => ({ _sum: { amount: 0 }, _count: 0 })),
    ...
    return NextResponse.json({
      ...
      donorOverview: {
        totalCollected: totalDonations._sum.amount || 0,
        monthCollected: monthDonations._sum.amount || 0,
        totalCount: totalDonations._count || 0,
        monthCount: monthDonations._count || 0,
        categoryCount: donationCategories.length || 1,
        recentCollections: recentDonations.map((d: any) => ({
          id: d.id,
          receiptNo: d.receiptNo || 'MP-REC',
          date: d.date,
          donorName: d.donorName || d.donor?.name || 'Valued Donor',
          donorPhone: d.donorPhone || d.donor?.phone || '',
          categoryName: d.category?.name || 'General Donation',
          fundName: d.fund?.name || 'General Fund',
          amount: d.amount,
